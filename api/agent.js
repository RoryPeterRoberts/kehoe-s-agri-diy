...
const TOOLS = [
  ...
  {
    name: 'save_memory',
    description: 'Save something you learned about the site owner — their preferences, style, corrections, or business context. This builds your memory over time so you can serve them better. Use this whenever you notice a pattern or receive a correction.',
    parameters: {
      type: 'object',
      properties: {
        category: {
          type: 'string',
          enum: ['style', 'content', 'design', 'business', 'correction'],
          description: 'Category: style (tone, voice), content (copy preferences), design (visual choices), business (context, seasonal info), correction (something they corrected you on)'
        },
        insight: {
          type: 'string',
          description: 'What you learned, written as a note to your future self. Be specific: "Prefers warm amber tones over cool blues" not "Likes warm colours".'
        }
      },
...
const DEFAULT_MEMORY = {
  ...
  preferences: {
    ...
    content: [],  // copy preferences, word choices
    design: [],   // visual choices, colours, layout
    business: [], // seasonal info, context, offerings
  },
  corrections: [],  // things the admin corrected — highest weight
  personality: {
    name: null,     // the agent can be given a name by the admin
    greeting: null, // personalised greeting learned over time
  }
};

// Merge a (possibly partial, legacy, or hand-edited) memory object over a fresh
// deep clone of the defaults. Guarantees an independent, fully-formed structure —
// never a shared reference to DEFAULT_MEMORY, and never missing a category key.
function normalizeMemory(parsed) {
  const base = JSON.parse(JSON.stringify(DEFAULT_MEMORY));
  if (!parsed || typeof parsed !== 'object') return base;
  return {
    ...base,
    ...parsed,
    interaction_count: parsed.interaction_count || 0,
    preferences: {
      ...base.preferences,
...
async function fetchMemory(gh) {
  try {
    const file = await gh.get(MEMORY_PATH);
    const content = Buffer.from(file.content, 'base64').toString('utf-8');
    return { memory: normalizeMemory(JSON.parse(content)), sha: file.sha };
  } catch {
    // No memory file yet — return a fresh, independent default
    return { memory: normalizeMemory(null), sha: null };
  }
}

async function saveMemoryFile(gh, memory, sha) {
  const content = JSON.stringify(memory, null, 2);
  const body = {
    message: 'BuildMySite: update agent memory',
    content: Buffer.from(content).toString('base64'),
    branch: gh._branch || 'main'
  };
  if (sha) body.sha = sha;
  const headers = gh._headers;
  const repo = gh._repo;
  const r = await fetch(
  ...
  if (!r.ok) {
    console.error('Failed to save memory:', r.status);
  }
  return r.ok;
}

function buildMemoryPromptSection(memory) {
  if (!memory || memory.interaction_count === 0) {
    return `\n## Your Memory\nThis is your first interaction with this site owner. Pay attention to their communication style, preferences, and any corrections. Use save_memory to record what you learn.\n`;
  }

  const sections = [];
  sections.push(`\n## Your Memory (${memory.interaction_count} previous interactions)`);

  if (memory.personality?.name) {
    sections.push(`You go by "${memory.personality.name}" with this admin.`);
  }

  // Corrections have highest weight — they represent direct feedback
  if (memory.corrections.length > 0) {
    sections.push('\n### Things They Corrected You On (IMPORTANT — do not repeat these mistakes)');
    // Show last 10 corrections
    const recent = memory.corrections.slice(-10);
    for (const c of recent) {
      sections.push(`- ${c.insight}`);
    }
  }

  // Preferences by category
  for (const [cat, prefs] of Object.entries(memory.preferences)) {
    if (prefs.length > 0) {
      const label = { style: 'Style & Tone', content: 'Content Preferences', design: 'Design Preferences', business: 'Business Context' }[cat] || cat;
      sections.push(`\n### ${label}`);
      // Show last 8 per category to keep prompt lean
      const recent = prefs.slice(-8);
      for (const p of recent) {
        sections.push(`- ${p.insight}`);
      }
    }
  }

  sections.push('\nContinue learning. Use save_memory when you notice new preferences or receive corrections.');
  return sections.join('\n');
}

// ---- Security helpers ---------------------------------------

const PROTECTED_PATHS = ['api/agent.js', 'api/contact.js', 'api/revert.js', 'api/site-info.js', 'admin.html', 'vercel.json', 'package.json', '.env', '.buildmysite/memory.json'];
...
// memoryState: { memory, sha, dirty } — shared mutable ref for in-flight memory updates
async function executeTool(name, args, env, pendingWrites, memoryState) {
  ...
  switch (name) {
    ...
    case 'save_memory': {
      ...
      if (!category || !insight) throw new Error('Both category and insight are required');

      const mem = memoryState.memory;
      const entry = { insight, date: new Date().toISOString().slice(0, 10) };

      if (category === 'correction') {
        mem.corrections.push(entry);
        // Keep last 20 corrections
        if (mem.corrections.length > 20) mem.corrections = mem.corrections.slice(-20);
      } else if (mem.preferences[category]) {
        mem.preferences[category].push(entry);
        // Keep last 15 per category
        if (mem.preferences[category].length > 15) {
          mem.preferences[category] = mem.preferences[category].slice(-15);
        }
      } else {
      ...
      return { saved: true, category, insight };
    }
...
function buildSystemPrompt(businessInfo, mode, memory) {
...
6. These structural CSS classes are validated before commit — if they are missing, the write will be rejected`;
  ...
  const memorySection = memory ? buildMemoryPromptSection(memory) : '';

  return `You are BuildMySite, an AI website builder and designer created by EON Risk Services.

You are not a generic chatbot. You are this site owner's personal web designer. You remember their preferences, learn from their corrections, and get better at serving them over time. Use save_memory whenever you learn something about their style, preferences, or business.
...
- Be warm and direct. Not corporate, not cutesy. Like a trusted freelancer who knows the business.
- Reference past interactions naturally: "Last time you mentioned..." or "Since you prefer..."
- When you spot a pattern in their requests, save it to memory and acknowledge it: "I've noted that you prefer..."
- If they correct you, acknowledge it graciously, save the correction, and never repeat the mistake.
- Adapt your communication style to match theirs — if they're brief, be brief. If they're detailed, be thorough.
...
async function runAgent(userMessage, env, businessInfo, mode) {
  ...
  // Fetch agent memory (personality, preferences, corrections)
  const { memory, sha: memorySha } = await fetchMemory(gh);
  const memoryState = { memory, sha: memorySha, dirty: false };

  // Increment interaction count
  memory.interaction_count = (memory.interaction_count || 0) + 1;
  memoryState.dirty = true;

  const systemPrompt = buildSystemPrompt(businessInfo, mode || 'modify', memory);
  ...
  for (let turn = 0; turn < 25; turn++) {
    ...
    for (const tc of response.toolCalls) {
      let result;
      try {
        result = await executeTool(tc.name, tc.args, env, pendingWrites, memoryState);
  ...
  // Save memory if it was modified during the interaction
  if (memoryState.dirty) {
    await saveMemoryFile(gh, memoryState.memory, memoryState.sha);
  }
  ...
  return {
    ...
    memory: {
      ...
      preferences_count: Object.values(memory.preferences).reduce((s, a) => s + a.length, 0),
      corrections_count: memory.corrections.length
    },
    usage: {
      model,
      provider,
